<template>
  <v-container>
      ...
      <v-card-text class="font-weight-bold text-center">
        <v-row class="px-1">
          <v-col
            cols="7"
            md="2"
            class="align-center d-flex caption font-weight-bold"
          >
            <v-row
            ...
            >
              ...
              <v-chip
                class="mt-2"
                small
                :color="showStatus(order.status).color"
              >
          ...
          <v-col cols="5" md="2" class="d-flex align-center">
            ...
            >
              قیمت کل
              <v-chip> {{ order.totalPrice | number }} تومان </v-chip>
            </v-row>
          </v-col>
          <v-col
            cols="12"
          ...
          <v-col cols="12" sm="8" md="5">
            <client-only>
              ...
              <template v-else>
                ...
                <v-btn
                  v-if="
                    ['empty', 'pending', 'reject'].indexOf(order.status) > -1
                  "
                  block
                  small
                  :class="
                    editingOrder._id == order._id || order.status == 'empty'
                      ? 'primary'
                      : 'info'
                  "
                  @click="submit(order)"
                ...
                <v-btn
                  v-if="['empty', 'pending'].indexOf(order.status) > -1"
                  class="error mt-2"
                  block
                  small
    ...
    <v-overlay v-model="paymentDialog" max-width="500">
      <v-card max-width="500">
        ...
        <v-card-text class="subtitle-2 text-center">
          ...
          شما پس از تایید حداکثر پس از 4 روز کاری تحویل شرکت پست می‌شود. پس از
          پرداخت و ثبت اطلاعات واریزی پیامک تایید را نهایتا تا یک روز کاری
          دریافت خواهید کرد
        </v-card-text>
        <v-card-actions>
          <v-btn
    ...
    <address-dialog v-model="addressDialog" :order="showingOrder" />
    <Delete
      v-model="deleteDialog"
      :name="`سفارش ${deletingOrder.trackingCode}`"
      :loading="loading"
      :item="deletingOrder"
...
export default {
  ...
  data () {
    return {
      ...
      deleteDialog: false,
      notEmpty: [v => !!v || ''],
      cardRules: [v => !!v || '', v => !v || v.toString().length == 4 || ''],
      showDate: false,
      paymentDialog: false,
      addressDialog: false,
      showingOrder: {},
