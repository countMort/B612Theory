<template>
  <v-container fluid>
    <v-row class="my-0">
      <v-col cols="12" sm="8">
        <v-card outlined>
          <v-card-title> مشاهده سفارش </v-card-title>
          <v-card-text>
            <v-row class="my-0">
              <v-col cols="12" sm="6">
              ...
              <v-col cols="12" sm="6">
                ...
                <v-divider />
                <v-radio-group v-model="paymentMethod" dense>
                  <v-radio
                    class="text-right"
                    ...
                    disabled
                  />
                  <v-radio class="text-right" label="کارت به کارت" :value="2" />
                </v-radio-group>
          ...
          <v-row class="my-0">
                  ...
                  <v-list-item v-for="product in getCart" :key="product.id">
                    <v-list-item-avatar :size="60" tile>
                      ...
                      <v-img
                        ...
                        :src="
                          product.photos &&
                            (product.photos[1] || product.photos[0])
                        "
                      />
                    </v-list-item-avatar>
                    <v-list-item-content>
            ...
            <v-col cols="12" sm="6">
              ...
              <v-radio-group
                ...
                class="mt-0 pt-2"
                dense
                @change="onChooseShipping"
              >
                <v-radio label="پیشتاز" value="normal" />
                <v-radio label="ویژه" value="fast" disabled />
              </v-radio-group>
              <v-alert
                v-show="deliveryMethod == 'fast'"
                class="subtitle-2 py-1 px-1 mb-0 accent--text text--darken-3"
              >
                در صورتی که ارسال پست ویژه به آدرس شما ممکن نباشد، مابه التفاوت
                مبلغ پست ویژه و پیشتاز به شما باز گردانده می‌شود.
              </v-alert>
            </v-col>
          </v-row>
        </v-card>
      ...
      <v-col cols="12" sm="4">
        <v-card class="mb-2 elevation-change">
