<template>
  <v-app class="mini_scroll_bar">
    ...
    >
      ...
      <v-toolbar-title class="pr-0" :class="$vuetify.breakpoint.xsOnly ? 'subtitle-1' : ''">
        <nuxt-link to="/">
        ...
        <template v-slot:activator="{attrs,on}">
          ...
          class="mx-auto">
            <v-badge overlap :badge="getCartLength" left>
              ...
              <v-icon :small="$vuetify.breakpoint.xsOnly">mdi-cart-outline</v-icon>
            </v-badge>
          </v-btn>
        </template>
        <span>
  ...
  methods: {
    ...
    menuGeneretor(context) {
      return [
          ...
          {
              text: (context.$auth.user && context.$auth.user.address) ? context.$auth.user.address.firstName : 'به کجا ارسال شود',
